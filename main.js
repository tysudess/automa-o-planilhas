...
function parseLine(line, isErr=false){
  ...
  if(line.includes("PROXY ATIVO")) stats.planilha = "PROXY ATIVO";
  ...
  if(line.includes("ERRO AO ENVIAR PARA PLANILHA") || line.includes("ERRO AO REGISTRAR VÍDEO")) { stats.planilha="ERRO"; stats.erros++; }
...
function iniciarMotor(){
  ...
  ensureConfig();
  encerramentoManual=false; stats.status="INICIANDO"; stats.whatsapp="CONECTANDO"; stats.planilha="AGUARDANDO"; send("status",stats);
  const env={...process.env,CONFIG_PATH:configPath()}; if(app.isPackaged) env.ELECTRON_RUN_AS_NODE="1";
  ...
  motor.on("exit",code=>{
    ...
    if(!encerramentoManual && code!==0){
      ...
      if(reiniciosAutomaticos<=5){ stats.status="RECUPERANDO"; send("status",stats); reinicioTimer=setTimeout(()=>{ reinicioTimer=null; iniciarMotor(); },5000); return; }
      stats.status="ERRO";
    } else stats.status="PARADO";
    if(stats.whatsapp!=="ERRO DE AUTENTICAÇÃO") stats.whatsapp="DESCONECTADO"; send("status",stats);
  });
...
app.whenReady().then(()=>{
  ...
  ipcMain.handle("motor:start",()=>iniciarMotor()); ipcMain.handle("motor:stop",()=>pararMotor()); ipcMain.handle("status:get",()=>stats);
...
app.on("before-quit",()=>{encerramentoManual=true;try{if(motor)motor.kill();}catch(_){} });
