...
async function postar(dados) {
  const r = await axios.post(APPS_SCRIPT_URL, dados, { headers:{"Content-Type":"application/json"}, timeout:15000 });
...
async function enviarParaPlanilha(message) {
  try {
    ...
    if (message.hasMedia && message.type === "video") {
      ...
      const resp = await postar({ aba:CONFIG.aba||"AGO26", data:video.data, grupo:"", veiculo:video.veiculo, titulo:video.titulo, autor:"Não Informado", analise:"", assunto:"", radar:"", link:"Vídeo" });
      if (resp?.sucesso) { console.log("VÍDEO REGISTRADO NA PLANILHA"); console.log("Linha:", resp.linha); }
      return;
    }
    ...
    console.log("Data:", data); console.log("Veículo:", noticia.veiculo); console.log("Título:", noticia.titulo); console.log("Autor:", noticia.autor || "não informado"); console.log("Análise:", analise); console.log("Assunto:", assunto); console.log("Link:", noticia.link);
    const resp = await postar({ aba:CONFIG.aba||"AGO26", data, grupo:"", veiculo:noticia.veiculo, titulo:noticia.titulo, autor:noticia.autor||"Não Informado", analise, assunto, radar:"", link:noticia.link });
    if (resp?.sucesso) { console.log("PLANILHA ATUALIZADA"); console.log("Linha:", resp.linha); }
...
client.on("message", async message => { try { await enviarParaPlanilha(message); } catch(e) { console.error(e); } });
client.on("message_create", async message => { try { if (message.fromMe) await enviarParaPlanilha(message); } catch(e) { console.error(e); } });
